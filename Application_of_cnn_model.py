"""
In this program I will use Convolutional Neural Network to perform recorded number prediction.
You can use this program to compare with a basic neural network model
"""
...
class App:
    ...
    def __init__(
        ...
        app_width: int = 500,
        app_height: int = 550,
        app_title: str = "Digit recognition with Convolutional Neural Network",
    ) -> None:
        """
        ...
        self.nn = tf.keras.models.load_model('mnist_cnn_model.h5')

    def read_btn(self) -> None:
        """
        Function to be called when the read button is clicked.
        """
        a = self._reshape_pixels() / 255
        a = self.nn.predict(a.reshape(1, 28, 28))
        prediction = a
    ...
    def _rescale_pixels(self) -> ndarray:
        ...
        a = array.reshape(sh).max(-1).max(1)
